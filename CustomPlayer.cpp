// CustomPlayer.cpp : 根据ffmpeg第七个教程，实现快进快退的功能。并包含了重采样的代码
...
void video_display(VideoState *is) 
{
	...
	if(vp->bmp) 
	{
		...
		rect.w = w;
		rect.h = h;
		SDL_DisplayYUVOverlay(vp->bmp, &rect);//Blit the overlay to the display,bmp:SDL_Overlay;
		/////
		 offset.x=420;
		 offset.y=430;
		 SDL_BlitSurface(pSurface,0,screen,&offset);
		 SDL_UpdateRect(screen,offset.x,offset.y,640-offset.x,480-offset.y);	//Makes sure the given area is updated on the given screen.
		//////
	}
}
...
int video_thread(void *arg) 
{
	...
	pFrame = avcodec_alloc_frame();//av_frame_alloc();
...
int main(int argc, char *argv[]) 
{
	...
	//如果要启动硬件加速，必须使用全屏模式（SDL_FULLSCREEN），
	//所以，在前面的软件渲染中，我们也使用全屏以作对比。第三，硬件渲染需要打开双缓冲（SDL_DOUBLEBUF），

	// Make a screen to put our video
#ifndef __DARWIN__
	screen = SDL_SetVideoMode(640, 480, 0, 0);//SDL_FULLSCREEN | SDL_DOUBLEBUF | SDL_HWSURFACE;
#else
	...
	SDL_WM_SetCaption(argv[1],0);//设置标题
	if(TTF_Init()<0)//装载扩展库：
	{
		fprintf(stdout,"TTF_Init() Error!");
	}
	else
	{
		pFont=TTF_OpenFont("/usr/share/fonts/liberation/LiberationMono-Bold.ttf",20);//打开字库
		...
		else
		{
			...
			textColor.g=0XFF;
			textColor.b=0XFF;
			pSurface=TTF_RenderText_Solid(pFont,"Liu Jiakun",textColor);//构建显示文本的surface
