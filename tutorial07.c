...
int iFrame=0;
//保存帧为图片
int save_frame(AVPicture pict,int width, int height)
{
    AVFormatContext* pFormatCtx;  
    AVOutputFormat* fmt;  
    AVStream* video_st;  
    ...
    char out_file[32];
	sprintf(out_file, "./JPEG/cuc_view_encode%d.jpg", ++iFrame);  
   ...
   int got_picture=0;  
    //编码  
    int ret = avcodec_encode_video2(pCodecCtx, &pkt,(AVFrame *)&pict, &got_picture);  
    if(ret < 0)  
    {  
        printf("编码错误！\n");  
        return -1;  
...
int stream_component_open(VideoState *is, int stream_index) 
{
	...
	switch(codecCtx->codec_type) 
	{
		...
		case AVMEDIA_TYPE_VIDEO:
			...
			is->sws_ctx =sws_getContext(is->video_st->codec->width,is->video_st->codec->height,is->video_st->codec->pix_fmt,
